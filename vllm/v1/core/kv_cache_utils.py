...
def _is_glm5_next_with_draft_swa(kv_cache_spec: dict[str, KVCacheSpec]) -> bool:
    """Whether this is the GLM-5.3-Flash hybrid carrying draft SWA layers.

    True when the Mamba/MLA/tail structure the GLM grouping keys on is all
    present besides the sliding-window specs, i.e. the layout is a GLM
    target with an external DFlash-style draft whose groups failed to join.
    """
    if not any(type(spec) is SlidingWindowSpec for spec in kv_cache_spec.values()):
        return False
    return any(isinstance(spec, MambaSpec) for spec in kv_cache_spec.values()) and all(
        type(spec) is MLAAttentionSpec
        for name, spec in kv_cache_spec.items()
        if not isinstance(spec, (MambaSpec, KpoolTailSpec, SlidingWindowSpec))
    )
...
def get_kv_cache_groups(
    ...
    if is_kv_cache_spec_uniform(kv_cache_spec):
        ...
        return glm5_groups
    elif _is_glm5_next_with_draft_swa(kv_cache_spec):
        # The GLM-5.3-Flash layout exists but the draft sliding-window specs
        # cannot join it (page too wide for the MLA slot). Falling through to
        # the generic path would promote the draft to full attention next to
        # the kpool indexer specs and silently break the pool-page alignment
        # the indexer asserts on, so fail fast instead.
        raise ValueError(
            "GLM-5.3-Flash with a sliding-window draft: the draft page does "
            "not fit the target MLA slot, so the draft KV groups cannot join "
            "the Mamba/MLA slot-sharing layout. Use a smaller draft KV head "
            "count or disable the speculative draft."
        )
